...
def _rrf(rank_lists: list[list[int]], weights: list[float], k0: int) -> dict[int, float]:
...
class BGEM3Index:
    def __init__(self, settings: Settings, load_model: bool = True) -> None:
        ...
        self.model = None
        self._document_chunk_lookup: dict[str, dict[int, dict]] = {}
        if load_model:
            self._load_model()
    ...
    def build(self) -> dict:
            ...
            json.dumps(
                {
                    ...
                    "max_length": self.settings.bge_max_length,
                    "retrieval": "dense+sparse weighted RRF; optional BGE-M3 pair scoring on candidates",
        ...
        self._rebuild_document_chunk_lookup()
    ...
    def _rebuild_document_chunk_lookup(self) -> None:
        """Build document_id -> chunk_index -> chunk lookup for final-answer neighbors."""
        ...
        for chunk in self.chunks:
            ...
            try:
                ...
                continue
            lookup.setdefault(str(document_id), {})[index] = chunk
        self._document_chunk_lookup = lookup
    ...
    def get_chunk_neighbors(
        ...
        """Return previous/next chunks from the same document.

        This is used only after final reranking to expand the TEXT context given to
        the answer model. It does not affect retrieval/reranking and does not add
        PDF images.
        """
        if radius <= 0:
            return [], []
        ...
        if not self._document_chunk_lookup and self.chunks:
            self._rebuild_document_chunk_lookup()

        by_index = self._document_chunk_lookup.get(str(document_id), {})
        previous = [
            by_index[idx]
            for idx in range(max(1, center - radius), center)
            if idx in by_index
        ]
        following = [
            by_index[idx]
            for idx in range(center + 1, center + radius + 1)
            if idx in by_index
        ]
    ...
    def search(self, query: str, top_k: int | None = None) -> list[SearchResult]:
        ...
        fused = _rrf(
            ...
            weights=[
                self.settings.rrf_dense_weight,
                self.settings.rrf_sparse_weight,
            ],
        ...
        if self.settings.use_bge_pair_rerank and candidates:
            try:
                ...
                score_out = self.model.compute_score(
                    ...
                    weights_for_different_modes=list(self.settings.bge_pair_mode_weights),
                )
